...
int main(){
      ...
      for(int i=0;i<size;i++)
        cin>>arr[i];
      int*** result = new int**[maxtrade+1];
      for(int j=0;j<=maxtrade;j++){  
          result[j]=new int*[2];
          result[j][0] = new int[size];
          result[j][1] = new int[size];
          for(int i=0;i<=size;i++){
            result[j][0][i]=-1;
            result[j][1][i]=-1;
          }
      }
      cout<<solution(arr,0,size-1,0,maxtrade,result)<<endl;
      for(int j=0;j<=maxtrade;j++){
        delete result[j][0];
        delete result[j][1];
        delete result[j];
